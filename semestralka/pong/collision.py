...
def bounce(ball, paddle, front_intersect, top_intersect):
    if front_intersect and top_intersect:
        # print(1)
        corner_bounce(ball)
...
def ball_paddle_collision(ball, paddle):
    ...
    if ball.y_velocity < 0:
        ...
        top_intersect = top_paddle_intersect(ball, paddle.pos_x, paddle.pos_y, paddle.width)
    
    bounce(ball, paddle, front_intersect, top_intersect)
    

def top_bottom_collision(ball, win):
    if ball.pos_y + ball.RADIUS >= win.get_height() or ball.pos_y - ball.RADIUS <= 0:
        ball.y_velocity *= -1