...
def deflection_angle(ball, paddle):
    ...
    relative_distance = ball_distance / (paddle.height/2 + ball.RADIUS)
...
def top_bounce(ball, paddle):
    ...
    if np.sign(ball.y_velocity) == np.sign(ball.pos_y - paddle.pos_y - paddle.height /2):
        ball.y_velocity = np.sign(ball.y_velocity)*5
    else:
        ball.y_velocity = np.sign(-ball.y_velocity)*5
